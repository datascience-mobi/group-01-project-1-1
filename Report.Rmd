...
In  this step we excluded the potential SSTs that do not cluster together with the corresponding DM based on the CERES scores of these genes in order to further reduce the number of potential SSTs based on their likeability.
We need to pay attention to the circumstance that the chance of putting a gene into the wrong cluster is about 50 % when 2 centers are used. The probability of wrong clustering is smaller if we used more centers. Based on our silhouette plot the optimal k seems to be 2 because there were less negative silhouette values compared to the clustering that was performed using 3 centers. Clustering with 2 centers also lead to 2 clearly distinct clusters. When we performed the k means clustering unsing 3 centers the result were two clusters that were very close and the separation was not as distinct. For this reason we chose to perform our k means with 2 centers.
