...
1b. Removing NA values from copy number matrix.
...
2. Scan for outliers via boxplots and threshold or remove them if necessary.
...
3a. Then we checked all matrices for containing values x = 0.
...
3b. Rows containing 0 in matrices were removed. 
...
barplot(common.genes, beside = T, names.arg = rownames(common.genes),  ylab = "Frequency", main = "All mutated genes", las = 2)
...
*6. PTEN: 17 mutations*
  Tumorsuppressorgene, which encodes for phosphatase and tensin homolog. The enzyme is involved in regulation of the cellcycle, preventing cells from growing and dividing too      much.
  Variations resullt in increased cell proliferation and reduced cell death. Frequent genetic inactivation of PTEN occurs in glioblastoma.
...
We visualized the gene´s distribution in all matrices by using heatmaps.
...
In order to just continue with the necessary data, we need to reduce our dataset. Since we already determined our most common mutations, we now isolate all celllines, which contain one of those specific mutations, from the mutation matrix. Our aim is to identify all other genes that were also mutated in the presence of this specific driving mutation (DM). Since there were a few thousand mutated genes for each DM, the ones were "IsDeleterious = TRUE" were extracted.
...
The definition of R-squared is the percentage of the response variable variation that is explained by a linear model. It is always between 0% and 100%. 0% means that the model explains none of the variability around its mean and 100% indicates that the model explains all the variability of the response data around its mean.
...
As we can see, the model for expression is not that great, having a R-squared value of 15,63%. The models for CERES score and probability look promising because of the values of 93,52% and 93,58%. The copy number model does not look good at all with a R-squared value of 0,34%.
