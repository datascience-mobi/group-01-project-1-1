...
* Two values were identified as outliers, then thresholded and removed them.
...
**1. MT-ND5:**
  ...
  Variations are associated with mitochondrial encephalomypathy, stroke-like episodes as well as Leighs syndrome and Leber's hereditary optic neuropathy.
  
Source: 
Whole-exome sequencing reveals a novel mutation of MT-ND5 gene in a mitochondrial cardiomyopathy pedigree: Patients who show biventricular hypertrophy, hyperlactacidemia, pulmonary hypertension, and decreased exercise tolerance (Zhou N, et al., 2019)
Impaired Cellular Bioenergetics Causes Mitochondrial Calcium Handling Defects in MT-ND5 Mutant Cybrids (McKenzie Mathew, Duchen Michael R; 2016)
...
Source:
Recessive truncating titin gene, TTN, mutations presenting as centronuclear myopathy (Ozge Ceyhan-Birsoy, *et al.*; 2013)
Mutations of TTN, encoding the giant muscle filament titin, cause familial dilated cardiomyopathy (Brenda Gerull, *et al.*; 2002)
...
Source:
Revisiting TP53 Mutations and Immunohistochemistry-A Comparative Study in 157 Diffuse Gliomas (Hirokazu Takami, *et.al*, 2014)
  
**4. MUC16:**
  (23 mutations) Gene, which encodes for the cancer antigen CA-125/mucin 16, a highly glycosylated protein. It has found application as tumor marker, since it plays a role in  tumor proliferation as a suppressor of the anti-tumor immune response.
  
Source:
CA125 in Ovarian Cancer (Nathalie Scholler, Nicole Urban; 2007)
...
**5. MT-CYB:**
  ...
  Variations result in mitochondrial deficiencies and associated disorders, including exericise intolerance.
  
Source:
A Novel in-Frame 18-bp Microdeletion in MT-CYB Causes a Multisystem Disorder with Prominent Exercise Intolerance (Carossa Valeria, *et. al*, 2014)
...
**6. PTEN:**
  ...
  Variations result in increased cell proliferation and in evasion of cell death. Frequent genetic inactivation of PTEN occurs in glioblastoma.
  
Source:
New insights into tumor suppression: PTEN suppresses tumor formation by restraining the phosphoinositide 3-kinase/AKT pathway (Lewis C. Cantley, Benjamin G. Neel; 1999)
...
In the next step we generated a correlation matrix including the correlation coefficients between all genes in the matrix and isolated the correlation coefficients between all genes and our specific DM.
...
Synthetic lethal targeting of PTEN-deficient cancer cells using selective disruption of polynucleotide kinase/phosphatase (Todd R. Mereniu; Mohamed A.M. El Gendy; Ana M. Mendes-Pereira)

* Known synthetic lethal interactions with PNKP, PARP and TKK, but neither of this genes appears in our correlation matrix.

Symbiotic Macrophage-Glioma Cell Interactions Reveal Synthetic Lethality in PTEN-Null Glioma (Peywen Chen *et al.*)
...
In this part we performed a k-means clustering using the CERES matrix to find out if our selected DMs and the potential SST are part of the same cluster. bla bla bla ... ergänzen

## First step: Define optimal number of clusters k

The CERES matrix was used to determine potential SSTs. In order to verify them we performed our k-means clustering using the CERES matrix as well.
...
As stated previously, the goal was to create a model that could predict one of our four variables using all three others (between expression, copynumber, CERES score and probability matrices). Our models proved not to be very useful or efficient in the prediction, especially for copy number and expression. The models for the prediction of the CERES score and probability looked promising becuase of the R-squared values, but after the visualisation of the predicted versus the real values, we can say that they are not suitable for an correct prediction.
