...
Our aim in this project was to find the driver mutations in GBM throughout all cell lines in our data set, compare them to the literature and investigate if there is a dependency between them and any second site targets. In the end the aim was to find a model where we could possibly predict either the expression, copy number, CERES score or the probability of stopping cell proliferation targeting one specific gene. The CERES score is a measure of how essential/important a particular gene is for cell survival. It reflects whether upon knocking down that genes the cell reduces or increases its proliferation or has no effect. Smaller values refer to higher essentiality.

# Data cleanup

Once we downloaded the data, we first removed all data that did not have to do with Glioblastoma and made the cell line identifiers be the rownames in the annotation.
...
1. Checking for NA values in all matrices.
...
**3. TP53:**
  (24 mutations) Tumor suppressing gene, which encodes for p53, which plays a role in regulation or progression through the cell cycle, apoptosis and genomic stability. 
...
According to scientific papers (McLendon et al., 2008; Brennan et al., 2013), the most prominent driving mutations for GBM are:
...
In order to just continue with the necessary data, we need to reduce our dataset. Since we already determined our most common mutations, we now manually isolate all cell lines from the mutation matrix, which contain one/all of those specific mutations. Since only PTEN, MUC-16 and TP53 are directly associated with the development of cancer, we decided wot continue working with these three driving mutations (DM). Our aim is to identify all other genes that were also mutated in the presence of these specific DMs. Since there were a few thousand mutated genes for each DM, the ones with "IsDeleterious = TRUE" were extracted.
...
In this part, we wanted to find potential second-site-targets in the GBM cell lines, where a specific driving mutation is present. After confirming the unnormal distribution of our data using Shapiro-Wilk-Tests, we used Spearman correlation to determine whether there is a dependency between the presence of a specific DM and the essentiality for cell survival of another mutated gene (indicated by low CERES score). We performed those tests with our cancer-associated DM TP53, PTEN and MUC-16. For each DM we found about 100 potential SSTs, but we could not confirm our results with scientific papers or the other way round, respectively. 
Furthermore, we are also not able to assign our cell lines to a specific GBM subtype (classical, mesenchymal, neural, proneural). However, we would assign our cell lines more towards the proneural or mesenchymal subtype, since these subtypes have mutations of PTEN and TP53 as their marker (Noushmehr et al., 2010; Verhaak et al., 2010). But we could not confirm any other known tumormarkers for those two subtypes in our cell lines.


# K-means

Using our reduced data set, we wanted to perform the kmeans clustering to find out if our selected DMs and the potential SST are part of the same cluster. If that is the case, the respective SSTs will be a valid target. 
...
## Creating CERES-dataframe

With k-means we are evaluating the CERES score of all cell lines, which have at least one of TP53, MUC-16 or PTEN and where the mutation has a functional effect. (Cell lines were manually selected, analogous to the code in Data Reduction.)
...
Goal of k means clustering is grouping of objects with similar values (here the CERES score) together in one specific cluster. Potential SSTs as well as our selected driver mutations should therefore be part of the same cluster. TP53, PTEN and MUC-16 are all part of cluster 2, so potential SSTs which belong to cluster 1 are removed.
We clustered our reduced data as described above and found the following potential SSTs in the same clusters as the DMs:

TP53

```{r}
genes.clusters = as.data.frame(km2.red$cluster)
genes.clusters$rownames = rownames(genes.clusters)
sst.genes.clusters.tp53 = genes.clusters[-(which(rownames(genes.clusters) %!in% rownames(SST.tp53))),]
sst.genes.p.clusters.tp53 = cbind(sst.genes.clusters.tp53, SST.tp53)
sst.genes.p.clusters.tp53 = sst.genes.p.clusters.tp53[, -2]
colnames(sst.genes.p.clusters.tp53) = c("Cluster", "Correlation to TP53")
sst.genes.p.clusters.tp53 = sst.genes.p.clusters.tp53[-which((sst.genes.p.clusters.tp53$Cluster) == 1),]
rownames(sst.genes.p.clusters.tp53)
```
...
genes.clusters = as.data.frame(km2.red$cluster)
genes.clusters$rownames = rownames(genes.clusters)
sst.genes.clusters.pten = genes.clusters[-(which(rownames(genes.clusters) %!in% rownames(SST.pten))),]
sst.genes.p.clusters.pten = cbind(sst.genes.clusters.pten, SST.pten)
sst.genes.p.clusters.pten = sst.genes.p.clusters.pten[, -2]
colnames(sst.genes.p.clusters.pten) = c("Cluster", "Correlation to PTEN")
sst.genes.p.clusters.pten = sst.genes.p.clusters.pten[-which((sst.genes.p.clusters.pten$Cluster) == 1),]
rownames(sst.genes.p.clusters.pten)
```
...
genes.clusters = as.data.frame(km2.red$cluster)
genes.clusters$rownames = rownames(genes.clusters)
sst.genes.clusters.muc16 = genes.clusters[-(which(rownames(genes.clusters) %!in% rownames(SST.muc16))),]
sst.genes.p.clusters.muc16 = cbind(sst.genes.clusters.muc16, SST.muc16)
sst.genes.p.clusters.muc16 = sst.genes.p.clusters.muc16[, -2]
colnames(sst.genes.p.clusters.muc16) = c("Cluster", "Correlation to MUC-16")
sst.genes.p.clusters.muc16 = sst.genes.p.clusters.muc16[-which((sst.genes.p.clusters.muc16$Cluster) == 1),]
rownames(sst.genes.p.clusters.muc16)
```

In this step we excluded the potential SSTs that do not cluster together with the corresponding DM based on the CERES scores of these genes in order to further reduce the number of potential SSTs based on their likelihood.
We need to pay attention to the circumstance that the chance of putting a gene into the wrong cluster is 50 % when 2 centers are used. The probability of wrong clustering is smaller if we used more centers. Based on our silhouette plot the optimal k seems to be 2, because there were less negative silhouette values compared to the clustering that was performed using 3 centers. Clustering with 2 centers also lead to 2 clearly distinct clusters. When we performed the k means clustering using 3 centers, the result were two clusters that were very close and the separation was not as distinct. For this reason we chose to perform our k-means with 2 centers.
A potential problem is that the TP53 is located very distant from all other data points. This affects the used method to determine the optimal number of k through WSS. WSS makes the assumption that clusters are convex and isotropic, which is not always the case. This method responds poorly to elongated clusters or manifolds with irregular shapes.

## Conclusion

Clustering of the CERES scores of all genes mutated in cell lines containing at least one of TP53, MUC-16 or PTEN, led to 2 distinct clusters with TP53 as an evident outlier. K-Means allowed to further reduce our set of possible SSTs to more accurately represent viable targets. As already stated above though, none of the SSTs identified in our subset of cell lines were found in current research.
...
As we can see, the models for expression and copy number do not look promising, having a R-squared value of 0.1563 and 0,0034. The models for CERES score and probability look more reliable with values of 0.9352 and 0.9358.
...
The R-squared value for the CERES score model was very promising, but judging by the visualization it is not reliable. Both real and predicted values peak in the same region, around 0.0, but they have very different densities and do not match. 
...
Looking at both curves we see the same problem we had with the CERES score model. The R-squared value looked promising but after training and testing our model, the result was not what we aimed for. The curves show that the predicted and real values are not similar, which proves our model to be not that good. 
...
Performing our data analysis we identified that three out of the six most common mutations across our cell lines were directly associated with cancer development (TP53, MUC-16 & PTEN). Two of those, TP53 and PTEN, are prominent genetic alterations in GBM.

Using the aforementioned mutations as driving mutations (DMs), we performed Shapiro-Wilk-Tests and used Spearman correlation to determine whether there are dependencies between the presence of a specific driving mutation and the essentiality for cell survival of another mutated gene. For each DM around 100 SSTs were found, none of which could be supported by scientific papers.

We could not assign our cell lines to a specific subtype of GBM. We would assign them more to the proneural or mesenchymal GBM subtypes, because of their specific markers, TP53 and PTEN. However, these were the only known markers for these two subtypes that we could identify in our cell lines.

Proceeding with the k-means clustering, we found that by the clustering of the CERES scores of all genes mutated in our cell lines containing at least one of our DMs, led to two distinct clusters. This method allowed us to further reduce our set of possible second site targets (SSTs), but none could be found in current research. 

We also did a multiple linear regression to create models to try and predict one of our four variables using all other three as a base. They proved not to be efficient for a prediction judging by the visualization. We expected the values for expression and copy number to be hardly predictable because these are such independent variables. The results for CERES score and probability were more pleasing. 

All in all, we did not find definite SSTs but did find possible targets that could be further researched with a bigger range of data and other cell lines, in order to confirm if they are indeed SSTs.
...
* The Cancer Genome Atlas Research, N., McLendon, R., Friedman, A., Bigner, D., Van Meir, E.G., Brat, D.J., M. Mastrogianakis, G., Olson, J.J., Mikkelsen, T., Lehman, N., et al. (2008). Comprehensive genomic characterization defines human glioblastoma genes and core pathways. Nature 455, 1061.
